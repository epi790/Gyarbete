...
def derive_new_key_from_time(shared_key):
    ...
    derived_key = HKDF(algorithm=hashes.SHA256(),length=64, salt=None, info=bytetime).derive(shared_key)
    return derived_key
...
def encrypt_AES256(message, key):
    iv = urandom(16)
    cipher = Cipher(algorithms.AES256(key), modes.CFB(iv), backend=crypto_default_backend())
